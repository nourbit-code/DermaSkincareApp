...
export default function PatientHistory() {
  ...
  const { name } = useLocalSearchParams(); // 🔹 receives ?name=Sara Ahmed from dashboard
  ...
  const selectedPatient = allPatients.find(p => p.name === name);

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Patient History</Text>

      {/* 🔹 Show selected patient summary if navigated from dashboard */}
      {selectedPatient && (
        <View style={styles.selectedCard}>
          <Text style={styles.selectedTitle}>Currently Viewing:</Text>
          <Text style={styles.selectedName}>{selectedPatient.name}</Text>
          <Text style={styles.selectedDetails}>Last Service: {selectedPatient.lastService}</Text>
          <Text style={styles.selectedDetails}>Last Visit: {selectedPatient.lastVisit}</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  selectedTitle: { fontSize: 14, color: '#9B084D', fontWeight: '600' },
  selectedName: { fontSize: 18, fontWeight: 'bold', color: '#9B084D', marginVertical: 4 },
  selectedDetails: { fontSize: 14, color: '#555' },
