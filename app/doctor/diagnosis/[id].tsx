...
// IMPORTANT: Assuming PatientInfoBar defines ServiceKey as type 'DIAGNOSIS' | 'PRESCRIPTION' | 'LASER' | 'OTHERS'
...
// ⭐️ NEW COMPONENT: Laser Content ⭐️
const LaserContent = ({ patient, diagnosis, saveData }: any) => {
    ...
    const handleSaveProtocol = () => {
        // In a real application, you would save this structured data to AsyncStorage or a remote database.
        const protocolData = { 
        ...
        console.log("Saving Laser Protocol:", protocolData);
        Alert.alert("Laser Data Saved", `Protocol for ${laserType} saved!`);
        // You might call saveData here to ensure all parent-level data is synced too
        // saveData(); 
    };

    return (
        <View style={[styles.card, { padding: 20 }]}>
            <SectionHeader icon="sparkles" title={`Laser Protocol for ${patient.name}`} color={THEME.danger} />
            
            <View style={styles.laserInfoBox}>
                <Text style={styles.label}>Current Diagnosis:</Text>
                <Text style={styles.diagnosisText}>{diagnosis || 'No Diagnosis Selected'}</Text>
            </View>

            <Text style={styles.label}>Laser Type / Wavelength</Text>
            <TextInput
                style={styles.input}
                placeholder="e.g., CO2 Fractional, 1064nm Nd:YAG"
                value={laserType}
                onChangeText={setLaserType}
            />

            <View style={styles.customInputRow}>
                <View style={{flex: 1}}>
                    <Text style={styles.label}>Fluence / Energy</Text>
                    <TextInput
                        style={styles.customInput}
                        placeholder="e.g., 20 J/cm², 100 mJ"
                        value={fluence}
                        onChangeText={setFluence}
                    />
                </View>
                <View style={{flex: 1}}>
                    <Text style={styles.label}>Spot Size / Density</Text>
                    <TextInput
                        style={styles.customInput}
                        placeholder="e.g., 5mm, 15%"
                        value={spotSize}
                        onChangeText={setSpotSize}
                    />
                </View>
            </View>

            <Text style={styles.label}>Procedure Notes (Anesthesia, Passes, Post-care)</Text>
            <TextInput
                style={[styles.input, styles.textarea, {height: 120}]}
                placeholder="Detailed notes on the procedure, pain management, number of passes, immediate reactions, and post-care instructions..."
                value={notes}
                onChangeText={setNotes}
                multiline
            />

            <TouchableOpacity style={styles.saveLaserBtn} onPress={handleSaveProtocol}>
                <Ionicons name="save" size={18} color={THEME.white} />
                <Text style={styles.saveBtnText}>Save Laser Protocol</Text>
            </TouchableOpacity>
        </View>
    );
};
...
  // ⭐️ Set initial active service for the default view
...
const renderContent = () => {
    // ⭐️ LOGIC TO SHOW LASER CONTENT WHEN ACTIVE ⭐️
    if (activeService === 'LASER') {
        return (
            <ScrollView 
                style={styles.fullColumnScroll}
            ...
            >
                <LaserContent 
                    patient={patient}
                    diagnosis={diagnosis}
                    saveData={saveData} // Pass general save function if needed
                />
            </ScrollView>
        );
    }
    
    // Default Content: DIAGNOSIS, PRESCRIPTION, etc. (Split View)
    return (
        <KeyboardAvoidingView 
        ...
        >
            ...
            >
                ...
                <TouchableOpacity style={styles.saveBtn} onPress={saveData}>
                    <Text style={styles.saveBtnText}>Save Visit Data</Text>
                </TouchableOpacity>
            </ScrollView>
...
// --- END CONDITIONAL RENDER FUNCTION ---
  
  return (
...
      {/* 2. SERVICE TABS - This controls the activeService state, triggering the view switch */}
      <ServiceTabs
        activeService={activeService}
        setActiveService={setActiveService}
      />

      {/* 3. MAIN CONTENT (Conditionally Rendered by renderContent) */}
      {renderContent()}
        
      {/* Modals */}
...
    paddingTop: 0, 
  },
  // ⭐️ NEW STYLE FOR FULL-WIDTH VIEW ⭐️
  fullColumnScroll: {
    flex: 1,
...
  },
  saveLaserBtn: {
    backgroundColor: THEME.danger, // Use danger color for laser save
    borderRadius: THEME.radius,
    padding: 12,
    flexDirection: 'row',
    justifyContent: 'center',
